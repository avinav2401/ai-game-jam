...
void main() {
  noise = detail * -.10 * turbulence( 0.6 * normal + (time * speed) + seed );
  float b = 2.0 * pnoise( 0.05 * position + vec3( 2.0 * (time * speed) ), vec3( 100.0 ) );
...
void main() {
  ...
  // Smooth edges by reducing opacity near the tip (high noise)
  float alpha = opacity * (1.0 - smoothstep(0.4, 0.8, range));
